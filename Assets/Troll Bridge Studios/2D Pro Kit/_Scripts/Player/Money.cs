...
namespace TrollBridge {
	...
	/// <summary>
	/// Money is a system we use to hold our currencies of all types.  Whether it be gold in World of Warcraft to Tokens you can spend to buy gear in World of Warcraft.
	/// </summary>
	public class Money : MonoBehaviour {
		...
		/// <summary>
		/// Adds the currency based on the currencyName.
		/// </summary>
		public void AddMoney(string currencyName, int amount){
			// Loop through all the currenies.
			for (int i = 0; i < currency.Length; i++) {
				// IF we find the currency we are looking for.
				if (currency [i].currencyName == currencyName) {
		...
		/// Subtracts the money.
		/// </summary>
		public void SubtractMoney (string currencyName, int amount) {
			// Loop through all the currenies.
			for (int i = 0; i < currency.Length; i++) {
				// IF we find the currency we are looking for.
				if (currency [i].currencyName == currencyName) {
