...
namespace TrollBridge {
	...
	/// <summary>
	/// Chest system in which we open a chest and get loot.
	/// </summary>
	public class Chest : MonoBehaviour {
		...
		public bool sendStraightToInventory = false;
		public Chest_Key_Consume ckc;
		[Tooltip("The force that is used to launch the item from the chest.")]
		...
		void Start () {
			...
			if (GetComponentInChildren<Icon_Display> () != null && GetComponent<SpriteRenderer> ().sprite == openChest) {
				// Set our icon inactive that is associated with this gameobject.
		...
		public void OpenChest (GameObject collidingObject) {
			...
			// Loop the amount of times we have items to spit out.
			for (int i = 0; i < chestLoot.Length; i++) {
				// IF we want to sent this straight to our inventory,
				// ELSE we want to shoot this gameobject out from the chest.
				if (sendStraightToInventory) {
					// Add the item to the inventory directly.
					Grid_Helper.inventory.AddItem (chestLoot [i].GetComponent<Item_GameObject> ().id, 1);
				} else {
					// Spawn the loot from the chest.
					GameObject chestGO = Grid_Helper.helper.SpawnObject (chestLoot[i], transform.position, Quaternion.identity, gameObject);
					// Launch the item in a direction away from the colliding object.
					Grid_Helper.helper.LaunchAwayFromPosition (chestGO, collidingObject.transform.position, forcePower);
				}
			}
