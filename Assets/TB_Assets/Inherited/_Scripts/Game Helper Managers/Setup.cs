...
namespace TrollBridge {
	...
	public class Setup : MonoBehaviour {
		...
		/// <summary>
		/// Is this Sprite in the resource folder?.
		/// </summary>
		...
		public Sprite GetSprite(string spriteName){
			// IF we have a blank sprite name.
			if (spriteName == "") {
				// We dont want to get a sprite so we leave.
				return null;
			}
		...
		/// <summary>
		/// IS this gameobject in our Resources folder?
		/// </summary>
		...
		/// <summary>
		/// Gets the game object prefab.
		/// </summary>
		public GameObject GetGameObjectPrefab(string gameobjectTitle){
			// Loop through the array size.
			for(int i = 0; 0 < gameobjects.Length; i++){
				// IF we have a matching pair.
				if(gameobjects[i].name == gameobjectTitle){
					// Return the GameObject
					return gameobjects [i];
				}
			}
			// We dun goofed.
			Debug.Log("The GameObject " +gameobjectTitle+ " is not in your Resources/Prefabs/ Folder.");
			return null;
		}
